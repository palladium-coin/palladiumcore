// Copyright (c) 2011-2020 The Palladium Core developers
...
PalladiumGUI::PalladiumGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
...
#ifdef ENABLE_WALLET
    if(enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this, &PalladiumGUI::showModalOverlay);
    }
#endif
...
void PalladiumGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Palladium address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and palladium: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &PalladiumGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(sendCoinsMenuAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsMenuAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &PalladiumGUI::gotoReceiveCoinsPage);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Palladium addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Palladium addresses"));
    ...
    openAction->setStatusTip(tr("Open a palladium: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Palladium command-line options").arg(PACKAGE_NAME));

    connect(quitAction, &QAction::triggered, qApp, QApplication::quit);
    connect(aboutAction, &QAction::triggered, this, &PalladiumGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &PalladiumGUI::optionsClicked);
    connect(toggleHideAction, &QAction::triggered, this, &PalladiumGUI::toggleHidden);
    connect(showHelpMessageAction, &QAction::triggered, this, &PalladiumGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &PalladiumGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &PalladiumGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &PalladiumGUI::setCurrentWallet);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &PalladiumGUI::setCurrentWallet);
...
void PalladiumGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, &PalladiumGUI::setCurrentWalletBySelectorIndex);
...
void PalladiumGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &PalladiumGUI::showProgress);
        ...
        if (optionsModel && trayIcon) {
            ...
            connect(optionsModel, &OptionsModel::hideTrayIconChanged, this, &PalladiumGUI::setTrayIconVisible);
...
#ifdef ENABLE_WALLET
void PalladiumGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    connect(wallet_controller, &WalletController::walletAdded, this, &PalladiumGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &PalladiumGUI::removeWallet);
...
void PalladiumGUI::addWallet(WalletModel* walletModel)
{
...
void PalladiumGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void PalladiumGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void PalladiumGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void PalladiumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void PalladiumGUI::setWalletActionsEnabled(bool enabled)
{
...
void PalladiumGUI::createTrayIcon()
{
...
void PalladiumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &PalladiumGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler *dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &PalladiumGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void PalladiumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void PalladiumGUI::macosDockIconActivated()
{
    show();
    activateWindow();
}
...
void PalladiumGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void PalladiumGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void PalladiumGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void PalladiumGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void PalladiumGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void PalladiumGUI::openClicked()
{
...
void PalladiumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void PalladiumGUI::gotoHistoryPage()
{
...
void PalladiumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void PalladiumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void PalladiumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void PalladiumGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Palladium network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void PalladiumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void PalladiumGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void PalladiumGUI::updateHeadersSyncProgressLabel()
{
...
void PalladiumGUI::message(const QString& title, QString message, unsigned int style, bool* ret)
{
...
void PalladiumGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &PalladiumGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &PalladiumGUI::show);
                e->ignore();
            }
        }
    }
...
void PalladiumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PalladiumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void PalladiumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(PalladiumUnits::formatWithUnit(unit, amount, true));
...
void PalladiumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PalladiumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool PalladiumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool PalladiumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void PalladiumGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void PalladiumGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void PalladiumGUI::updateWalletStatus()
{
    if (!walletFrame) {
        return;
    }
...
void PalladiumGUI::updateProxyIcon()
{
    std::string ip_port;
...
void PalladiumGUI::updateWindowTitle()
{
...
void PalladiumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void PalladiumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PalladiumGUI::detectShutdown()
{
...
void PalladiumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void PalladiumGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void PalladiumGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(PalladiumGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void PalladiumGUI::subscribeToCoreSignals()
{
...
void PalladiumGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(nullptr)
{
    ...
    QList<PalladiumUnits::Unit> units = PalladiumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const PalladiumUnits::Unit unit : units)
    {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, PalladiumUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const PalladiumUnits::Unit u : PalladiumUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(PalladiumUnits::longName(u)), this);
...
{
    setText(PalladiumUnits::longName(newUnits));
}
