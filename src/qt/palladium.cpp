...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. palladium_de.qm (shortcut "de" needs to be defined in palladium.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. palladium_de_DE.qm (shortcut "de_DE" needs to be defined in palladium.qrc)
...
void PalladiumApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the PalladiumApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", PalladiumGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void PalladiumApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

void PalladiumApplication::createOptionsModel(bool resetSettings)
{
    optionsModel = new OptionsModel(m_node, nullptr, resetSettings);
}

void PalladiumApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new PalladiumGUI(m_node, platformStyle, networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &PalladiumGUI::detectShutdown);
}
...
void PalladiumApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    ...
    splash->show();
    connect(this, &PalladiumApplication::splashFinished, splash, &SplashScreen::finish);
    connect(this, &PalladiumApplication::requestedShutdown, splash, &QWidget::close);
}

bool PalladiumApplication::baseInitialize()
{
...
void PalladiumApplication::InitializePruneSetting(bool prune)
{
...
void PalladiumApplication::requestInitialize()
{
...
void PalladiumApplication::requestShutdown()
{
...
void PalladiumApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        // palladium: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PalladiumGUI::handlePaymentRequest);
            connect(window, &PalladiumGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void PalladiumApplication::shutdownResult()
{
    quit(); // Exit second main loop invocation after shutdown finished
}

void PalladiumApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(nullptr, "Runaway exception", PalladiumGUI::tr("A fatal error occurred. Palladium can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId PalladiumApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", PalladiumGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(palladium);
    Q_INIT_RESOURCE(palladium_locale);
    ...
    /// 6. Determine availability of data directory and parse palladium.conf
    ...
    // palladium: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
